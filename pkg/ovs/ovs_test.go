...
func TestTransactionSuccess(t *testing.T) {
	exec.AddTestResult("/usr/bin/ovs-ofctl add-flow br0 flow1", "", nil)
	exec.AddTestResult("/usr/bin/ovs-ofctl add-flow br0 flow2", "", nil)
...
func TestTransactionFailure(t *testing.T) {
	exec.AddTestResult("/usr/bin/ovs-ofctl add-flow br0 flow1", "", fmt.Errorf("Something bad happened"))
...
func TestDumpFlows(t *testing.T) {
	exec.AddTestResult("/usr/bin/ovs-ofctl dump-flows br0", `OFPST_FLOW reply (OF1.3) (xid=0x2):
